/*
  # Fix Name Construction in User Creation Trigger

  1. Problem
    - The handle_new_user function expects a single 'name' field in metadata
    - The signup form sends 'firstName' and 'lastName' separately
    - This causes the trigger to fail when creating user profiles

  2. Solution
    - Update the trigger function to construct name from firstName + lastName
    - Keep fallback to single 'name' field for backward compatibility
    - Ensure proper trimming and handling of empty values

  3. Changes
    - Replace the name extraction logic in handle_new_user function
    - Use CONCAT to combine firstName and lastName with proper spacing
    - Add TRIM to remove extra whitespace
*/

-- Update the handle_new_user function to properly construct names
