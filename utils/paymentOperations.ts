...
export interface PaymentMethodService {
  ...
  removePaymentMethod: (paymentMethodId: string, stripePaymentMethodId: string) => Promise<{ success: boolean; error?: string }>;
}
...
export const paymentMethodService: PaymentMethodService = {
  ...
  removePaymentMethod: async (paymentMethodId: string, stripePaymentMethodId: string) => {
...
export const requestService: RequestService = {
  createRequestWithPayment: async (requestData: any, userId: string) => {
    try {
      ...
      if (!paymentResult.success) {
        ...
        if (paymentResult.error?.includes('Network error') || paymentResult.error?.includes('Failed to fetch')) {
          console.warn('Backend not accessible, proceeding without payment for testing purposes');
          ...
          if (errorMessage.includes('No such PaymentMethod')) {
            errorMessage = 'Your payment method is invalid or expired. Please add a new payment method in your profile and try again.';
          } else if (errorMessage.includes('card_declined') || errorMessage.includes('declined')) {
      ...
      if (transactionError) {
        console.error('Error creating payment transaction record:', transactionError);
  ...
  acceptRequestWithPayment: async (requestId: string, providerId: string) => {
    try {
      ...
      console.log('Attempting to charge provider for acceptance:', { providerId, requestId });
      const paymentResult = await chargeProviderForAcceptance(providerId, requestId);
      ...
      console.log('Updating request status to accepted:', { requestId, providerId });
