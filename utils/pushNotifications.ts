...
// if (__DEV__) {
//   global.testUserNotification
//    = async (userId: string = '28d8549c-b466-4e69-9356-88cb46fd88a9') => {
 ...
 */
export async function testPushNotificationDelivery(
  userId: string,
): Promise<void> {
  try {
    ...
    console.log(
      '📝 Token preview:',
      userData.push_token.substring(0, 50) + '...',
    );
    ...
    if (result.data && result.data[0]) {
      ...
      if (notificationResult.status === 'ok') {
        ...
        console.log(
          '📱 Check device for notification (may take a few seconds)',
        );
      } else {
        console.log(
        ...
        console.log(
          '🔍 Details:',
          notificationResult.details || 'No details available',
        );
      }
    } else {
...
export async function testUserNotification(
  userId: string = '28d8549c-b466-4e69-9356-88cb46fd88a9',
): Promise<void> {
...
export async function debugCheckPushToken(userId: string): Promise<void> {
  try {
    ...
    if (data.push_token) {
      console.log(
        '  - Token Preview:',
        data.push_token.substring(0, 50) + '...',
      );
    }
  } catch (error) {
...
export async function initializePushNotifications(
  ...
  try {
    ...
    console.log(
      '📋 Existing push token in DB:',
      existingToken ? 'present' : 'none',
    );
    ...
    if (token) {
      ...
      console.log(
        '❌ No push token available - permissions denied or device issue',
      );
