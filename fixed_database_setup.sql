-- Fixed RigSnap Database Setup Script
-- Run this in your Supabase SQL Editor to create the complete database schema
...
CREATE INDEX idx_requests_trucker_id ON public.requests(trucker_id);
CREATE INDEX idx_requests_provider_id ON public.requests(provider_id);
...
CREATE INDEX idx_leads_request_id ON public.leads(request_id);
...
-- 9. Drop all existing policies to avoid conflicts
...
-- Permissive INSERT policy that allows trigger functions to work
...
-- 13. Create trigger for users table updated_at
...
-- 14. Drop existing user creation function and trigger
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS handle_new_user();

-- 15. Create the user creation function with proper name construction
...
-- 18. Verify everything was created successfully
SELECT 'Checking tables...' as status;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('users', 'requests', 'messages', 'leads');
